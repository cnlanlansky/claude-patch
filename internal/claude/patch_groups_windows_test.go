...
func TestCurrentClaude237PatchGroupsSmoke(t *testing.T) {
	if os.Getenv("CLAUDE_PATCH_LIVE_ACCEPTANCE") != "1" {
		t.Skip("设置 CLAUDE_PATCH_LIVE_ACCEPTANCE=1 才运行隔离 2.1.237 patch group smoke")
	}
	...
	groups := []struct {
		...
		{name: "picker-context-fast", specs: patchSpecs237[:5]},
		{name: "project-client", specs: patchSpecs237[5:6]},
		{name: "subagent", specs: patchSpecs237[6:11]},
		{name: "team", specs: patchSpecs237[11:]},
	}
